var ActiveUser = function(user_id, location, chat_styles, socket){
	this.user_id = user_id;
	this.location = location;
	this.socket = socket;
	this.updateNickname('user-' + this.user_id);
...
ActiveUser.prototype.updateNickname = function(nickname){
	...
	this.socket.emit('nickname_accepted', {nickname: nickname});
};

ActiveUser.prototype.announcePresenceOf = function(user){
	this.socket.emit('room_enter', {nickname: user.getNickname()});
};

ActiveUser.prototype.announceDepartureOf = function(user){
	this.socket.emit('room_leave', {nickname: user.getNickname()});
};

ActiveUser.prototype.announceNicknameChange = function(prev_nickname, new_nickname){
	this.socket.emit('nickname_change', {prev_nickname: prev_nickname, new_nickname: new_nickname});
};
